...
import { FindEndpoint, Sending_discord_wh, Sending_email, Sending_slack_wh, StoringData } from "./actions"
...
export async function POST(req: Request , { params }: { params: { slug: string } }) {
    try {
     ...
     // FETCHING ENDPOINT 
       ...
       //STORING N DB 
       const storing = await StoringData(body)
       const formdata = body
       Sending_slack_wh(formdata , slack_wh)
       Sending_discord_wh(formdata , discord_wh)
       Sending_email(formdata , email ,endpoint)
        //returning response 
